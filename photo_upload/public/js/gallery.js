...
function adjust() {
  ...
  for (var index = 0; index < boxes.length; index++) {
    ...
    if (recent_width >= $("#gallery").width() - $(boxes[index]).height() / 2) {
      ...
      for (var i = 0; i <= recent_num; i++) {
        ...
        var picIntro = $("<div class = 'single-line pic-intro'></div>").text("lalala");
  ...
  $("#gallery .box div").hover(function(){
  	var picTitle = $("<div class = 'pic-title'></div>").text("lalala");
  	var picContent = $("<div class = 'pic-content'></div>").text("biuibiubiubiuheiheiheibiubiubiubiubiu");
  	$(this).empty();
